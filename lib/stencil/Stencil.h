...
public:
    ...
    // Constructor
    ////////////////////////////////////////
    CartesianStencil(GridBase *grid, int npoints, int checkerboard, const std::vector<int> &directions, const std::vector<int> &distances) :
        ...
        _comm_buf_size(npoints)
    {
        ...
        for(int ii=0;ii<npoints;ii++)
        {
            ...
            // Should not be used with extended simd implementation for Aurora-SX
//             _permute_type[point] = _grid->PermuteType(dimension);
            ...
            bool comm_dim       = _grid->_processors[dimension] > 1;
            bool splice_dim     = _grid->_simd_layout[dimension] > 1 && (comm_dim);
            bool rotate_dim     = _grid->_simd_layout[dimension] > 2;
    ...
    // Stencil query
    ////////////////////////////////////////
    inline bool SameNode(int point) { 
    ...
    template<class compressor> void HaloExchange(const Lattice<vobj> &source,compressor &compress) 
    {
        ...
        Prepare();                      // Resize containers to 0
        HaloGather(source,compress);    // redirects to HaloGatherDir in for each point
    ...
    bool HaloGatherDir(const Lattice<vobj> &source,compressor &compress,int point,int & face_idx)
    {
        int dimension    = _directions[point];
        int displacement = _distances[point];
    ...
    void HaloGather(const Lattice<vobj> &source,compressor &compress)
    {
        ...
        {
            compress.Point(point); // Does nothing
...
private:
    ...
    template<class compressor>
    bool Gather(const Lattice<vobj> &rhs,int dimension,int shift,int cbmask,compressor & compress,int &face_idx)
    {
        typedef typename cobj::vector_type vector_type;
        typedef typename cobj::scalar_type scalar_type;
        ...
        {       
            int rplane        = (lplane+sshift)%rd;
            int comm_proc = ((lplane+sshift)/rd)%pd;
            
            if (comm_proc != 0) // if one have to communicate with another processor
            {
                int words = buffer_size;
                if (cbmask != 0x3) words=words>>1;
                ...
                int so  = rplane*rhs._grid->_ostride[dimension]; // base offset for start of plane 
                if ( !face_table_computed ) 
                {
                    face_table.resize(face_idx+1);
                    Gather_plane_table_compute ((GridBase *)_grid,dimension,rplane,cbmask,u_comm_offset,face_table[face_idx]);
                }
                ...
                if ( compress.DecompressionStep() ) recv_buf = u_simd_recv_buf[0];
                else                                recv_buf = u_recv_buf_p; // set to simd send buffer

                send_buf = (cobj *)_grid->ShmBufferTranslate(xmit_to_rank,recv_buf);
                
                if( send_buf==nullptr ) send_buf = u_send_buf_p;
                ...
                if ( compress.DecompressionStep() ) 
                {
                    if ( shm_receive_only ) 
                    { 
                        // Early decompress before MPI is finished is possible
                        AddDecompress(&u_recv_buf_p[u_comm_offset], &recv_buf[u_comm_offset], words,DecompressionsSHM);
                    } 
                    else
                    { 
                        // Decompress after MPI is finished
                        AddDecompress(&u_recv_buf_p[u_comm_offset], &recv_buf[u_comm_offset], words,Decompressions);
                    }
